    ...
    if (s_currentPlayLayer) {
        ...
        if (s_currentPlayLayer->m_fields->m_cancelLevelLoad) {
            ...
            if (l_mainLayer) {
                ...
                // Re-enable touch on LevelSelectLayer itself so keyboard-nav buttons work
                // (replaces the raw DIDCLICK_OFFSET memory write from 2.2074)
                l_mainLayer->setTouchEnabled(true);
            }
