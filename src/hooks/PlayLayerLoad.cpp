    ...
    switch (m_fields->m_loadingState) {
        case LoadingState::Setup: {
            ...
            if (!l_validSaveExists && Mod::get()->getSettingValue<bool>("disable-popup-on-new-game")) {
                ...
                m_fields->m_newGameSlot = 0;
                m_fields->m_loadingState = LoadingState::Ready;
                break;
            } else {
                bool l_slotExists[PS_SAVE_SLOT_COUNT];
                for (int i = 0; i < PS_SAVE_SLOT_COUNT; i++) {
                    l_slotExists[i] = util::filesystem::getSaveFilePath(m_level, i, true) != "";
                }
                PlayLevelMenuPopup* l_playLevelMenuPopup = PlayLevelMenuPopup::create(l_slotExists);
        ...
        case LoadingState::WaitingForPlayLevelMenuPopup: {
            ...
            if (m_fields->m_saveSlot == -1) {
                ...
                // New game: write to the slot the popup chose
                m_fields->m_saveSlot = m_fields->m_newGameSlot;
                m_fields->m_loadingState = LoadingState::Ready;
                break;
            }
            else {
