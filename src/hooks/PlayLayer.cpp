...
void PSPlayLayer::processCreateObjectsFromSetup() {
    // PAPlayLayer::init (PersistenceAPI v1.2.0) sets m_uniqueIDBase by reading
    // the engine's global object-ID counter before any objects are created, so
    // we no longer need to reset the counter via a version-specific raw offset.
